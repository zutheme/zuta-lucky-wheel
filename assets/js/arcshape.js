...
class Arcshape {
  ...
  triggerWin() {
      ...
      console.log(`LTW Debug: Arcshape triggerWin called for ${this.label}`);
      ...
      // --- SỬA LỖI Ở ĐÂY ---
      // Kiểm tra xem cầu nối window.ltw_show_win có tồn tại không
      console.log("LTW Debug: Checking for popup function...");

      if (typeof window.ltw_show_win === 'function') {
           console.log("LTW Debug: Found window.ltw_show_win. Calling it now.");
           setTimeout(() => {
               window.ltw_show_win(this.label);
           }, 100);
      } else if (typeof get_popup === 'function') {
           // Fallback nếu get_popup vô tình được khai báo global (ít khi xảy ra)
           console.log("LTW Debug: Found direct get_popup.");
           setTimeout(() => {
               get_popup(this.label, this.strurl, _idcampain);
           }, 100);
      } else {
           console.error("LTW Error: Neither 'window.ltw_show_win' nor 'get_popup' functions are defined!");
