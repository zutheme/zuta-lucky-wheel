// =======================================================
//  SKETCH.JS - Final Version (Sound Volume Fix)
// =======================================================
...
// ===================== GUARD & CONFIG =====================
if (typeof window.LuckyWheelFront === 'undefined') {
...
let load = document.getElementsByClassName("ltw-popup-processing")[0];
    ...
    try {
        if (typeof loadSound === 'function') {
            startsound = loadSound(_startsound, 
                () => { console.log("LTW Sound: Spin sound loaded successfully."); }, 
                (err) => { console.warn("LTW Sound: Failed to load spin sound.", err); startsound = makeSoundStub(); }
            );
            winsound = loadSound(_winsound, 
                () => { console.log("LTW Sound: Win sound loaded successfully."); }, 
                (err) => { console.warn("LTW Sound: Failed to load win sound.", err); winsound = makeSoundStub(); }
            );
...
// ===================== P5 SETUP =====================
function setup() {
    console.log("LTW Debug: Setup started");
    ...
    if (container) {
        ...
        if (!loadconfig) {
            getdatascore(function(data) {
                console.log("LTW Debug: Config loaded via AJAX");
                restart_game(data);
            });
            loadconfig = true;
        }
        ...
        if (http.readyState === 4) {
            if (http.status === 200) {
                try {
                    ...
                    dataconfig = arr; 
                    callback(arr);
                } catch (e) { console.error('LTW Error: Invalid JSON Config', e); }
            } else {
                console.error('LTW Error: Failed to load config. Status:', http.status);
            }
...
function restart_game(data) {
    console.log("LTW Debug: restart_game called");
    ...
    try {
        ...
        if (typeof startsound !== 'undefined' && startsound) {
            ...
            console.log("LTW Sound: Volume reset to 1.0 (Instant) in restart_game");
        }
    } catch(e) { console.warn("LTW Sound Reset Error", e); }
    // ----------------------------------------------
    ...
    let response;
    try { response = JSON.parse(data[0].dataconfig); } catch (e) { console.error("LTW Error parsing dataconfig inner JSON", e); return; }
    ...
    for (let bt of buts) {
        ...
        if (bt && typeof bt.clicked === 'function' && bt.clicked() && !s_rotated && valid > 0) {
            ...
            const runServerCheck = (recaptchaToken = '') => {
                ...
                console.log("LTW Debug: Requesting spin result from Server...");
                ...
                jQuery.ajax({
                    url: window.LuckyWheelFront.ajax_url,
                    type: 'POST',
                    data: data,
                    success: function(response) {
                        ...
                        if (response.success) {
                            ...
                            console.log("LTW Debug: Server result received. Target Segment:", response.data.segment_id);
                            ...
                            // --- ERROR / DENIED ---
                            console.warn("LTW Debug: Server Denied - " + (response.data ? response.data.message : 'Unknown error'));
                            let msg = (response.data && response.data.message) ? response.data.message : "Error processing spin.";
                            if (typeof ltw_show_notice === 'function') {
                                ltw_show_notice(msg);
                            } else {
                                alert(msg);
                            }
                    ...
                    error: function(err) {
                        window.isCheckingServer = false;
                        document.body.style.cursor = 'default';
                        console.error("LTW Error: Could not connect to server", err);
                    }
                });
            };

            // Recaptcha handling (kept from your original code)
            const siteKey = window.LuckyWheelFront.recaptcha_site_key;
...
function start_physics_spin(targetIndex) {
    ...
    console.log(`LTW Physics: Target Index ${idx}, Distance ${totalDistance.toFixed(2)}, Speed set to ${speed.toFixed(5)}`);
}
        ...
        if (typeof p5 !== 'undefined' && p5 && typeof p5.SoundFile === 'function') {
            ...
            try { 
                win = new p5.SoundFile(_plugin_dir + "sound/win1.mp3", 
                    () => console.log("LTW Sound: Base Win sound loaded")
                ); 
